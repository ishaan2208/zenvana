...
- **Payments** run through Razorpay: `src/app/api/razorpay/order/route.ts` creates orders server-side. Amounts are converted to paise (`amount * 100`) before hitting Razorpay — INR is stored/handled in rupees in app code.
...
- `RAZORPAY_KEY_ID` may be public; the secret may not. Order creation must stay on the server route.
